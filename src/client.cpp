      ...
      socket_(io_service, context)
{
    ...
    resolver_.resolve(query,           boost::bind(&client::handle_resolve,this, boost::asio::placeholders::error                                 boost::asio::placeholders::iterator));
}
                    ...
                    tcp::resolver::iterator endpoint_iterator)
{
    if (!err)
    {
        ...
        boost::asio::connect(socket_.lowest_layer(), endpoint_iterator,      boost::bind(&client::handle_connect, this,
...
void client::handle_connect(const boost::system::error_code& err)
{
    ...
    {
        std::cout << "Connect OK " << "\n";
        socket_.handshake(boost::asio::ssl::stream_base::client, boost::bind(&client::handle_handshake, this, boost::asio::placeholders::error));
    }
    else
    {
...
void client::handle_handshake(const boost::system::error_code& error)
{
    if (!error)
    {
        ...
        boost::asio::write(socket_, request_, boost::bind(&client::handle_write_request, this, boost::asio::placeholders::error));
    }
    else
    {
...
void client::handle_write_request(const boost::system::error_code& err)
{
    if (!err)
    {
        ...
        boost::asio::read_until(socket_, status_, "\r\n",
...
void client::handle_read_status_line(const boost::system::error_code& err)
{
    if (!err)
    {
        ...
        std::istream response_stream(&status_);
...
void client::handle_read_headers(const boost::system::error_code& err)
{
    if (!err)
    {
        ...
        std::istream response_stream(&headers_);
        std::string header;
        ...
        boost::asio::read(socket_, content_, boost::bind(&client::handle_read_content, this, boost::asio::placeholders::error));
    }
    else
    {
        std::cout << "Error: " << err << "\n";
...
void client::handle_read_content(const boost::system::error_code& err)
{
    if (!err)
    {
        ...
        boost::asio::read(socket_, content_,
...
std::string client::getContent()
{
	...
	auto bufs = content_.data();
	std::string result(buffers_begin(bufs), buffers_begin(bufs) + content_.size());
	return result
}
/*
